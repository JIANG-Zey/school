myFocus.pattern.extend({//*********************51xflash(仅支持4帧)******************
	'mF_51xflash':function(settings,$){
		...
		$focus.play(function(prev,n){
			...
			$picList.eq(prev).slide({right:0,top:parseInt($picList[next].style.top),width:w,height:h},2,function(){this.style.zIndex=''});
			$picList.eq(next).slide({right:disX,top:0,width:settings.width,height:settings.height},2);
